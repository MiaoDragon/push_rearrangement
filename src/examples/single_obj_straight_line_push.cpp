                   ...
                   Vector6d& twist)
{
    ...
    double d_pos = pos.norm();
    double duration = std::max(d_pos / v_max, angle / r_max);

    twist.head(3) = pos / duration;
    twist.tail(3) = angle / duration * axis;
}
                   ...
                   Vector6d& twist)
{
    ...
    Vector3d pos, axis;
    double angle;
    pose_to_transform(start_p, start_r, goal_p, goal_r, pos, angle, axis);
    double d_pos = pos.norm();
    double duration = std::max(d_pos / v_max, angle / r_max);
...
void straight_line_plan_1(Vector3d& robot_pos)
{
    ...
    Vector6d twist;
    pose_to_twist(start_T, goal_T, twist);
    ...
    for (int i=0; i<contacts.contacts.size(); i++)
    {
        ...
        Vector3d twist1 = Vector3d::Zero();
        Vector3d twist2 = Vector3d::Zero();
        if (contacts.contacts[i]->body_type1 == BodyType::OBJECT)  twist1 = twist;
        if (contacts.contacts[i]->body_type2 == BodyType::OBJECT)  twist2 = twist;

        int cs_mode;
        std::vector<std::vector<int>> ss_mode;
...
void sample_robot_pos_loop()
{
    ...
    while (true)
    {
        ...
        int status = straight_line_plan_1(res);
...
int main(void)
{
    ...
    sample_robot_pos_loop(robot_pos);
